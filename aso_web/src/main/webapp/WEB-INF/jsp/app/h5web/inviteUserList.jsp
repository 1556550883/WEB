...
<body style="background:#F0F0F0; margin:0px;font-size:20px; font-family:微软雅黑;">
       <div onclick="go()"  class="title">
            ...
            <span style=".flex1; line-height:0.8rem; font-weight: bold; color: #4a4a4a; font-size: 0.4rem;margin:auto;position: absolute;top: 0;  left: 0;right: 0;bottom: 0">邀请的人</span>
    </div>
    
    <div id="container" style="padding-top:0.9rem;position:relative;margin-left:2%;width:96%;"></div>
	<script>
		(function(){
			var result = ${userAppList};
			var adapter = new C3ListView.adapter(result);
			adapter.getHtml = function(position){
				var data = result[position];
				var name =  data.userAppId;
				var src = "../img/h5web/happy_logo.png";
				if(data.weixin && data.isEffective == 0){
					name = data.weixin +"  id="  + data.userAppId;
					src = data.flag5;
				}
				...
				//else if(data.weixin && data.zhifubao && data.phoneNum && data.isEffective == 1){
				//	status = "黑名单用户";
				//}
